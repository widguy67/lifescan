...
export function ScanResult({
  ...
  const handleShare = async () => {
    const text = `${record.commonName}${record.scientificName ? ` (${record.scientificName})` : ""} — identified with Scany at ${record.confidence}% confidence.\n\n${record.summary}`;
    try {
      if (navigator.share) {
        await navigator.share({ title: `Scany — ${record.commonName}`, text });
      } else {
    ...
    <div className="space-y-6 animate-float-up">
      <div className="overflow-hidden rounded-3xl border border-border bg-card shadow-elegant">
        <div className="relative">
          <img
            src={record.image}
            alt={`${record.commonName} — identified by Scany`}
